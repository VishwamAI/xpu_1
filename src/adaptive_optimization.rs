...
impl AdaptiveOptimizer {
    pub fn new() -> Self {
        AdaptiveOptimizer { iteration: 0 }
    }

    pub fn optimize(
        &mut self,
        metrics: &OptimizationMetrics,
    ) -> Result<OptimizationParams, XpuOptimizerError> {
        self.iteration += 1;
        ...
        println!(
            "Iteration {}: Adjusting parameters based on current metrics",
            self.iteration
        );
        println!(
            "Average load: {:.2}, Average latency: {:?}",
            metrics.average_load, metrics.average_latency
        );
